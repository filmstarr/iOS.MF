...
function template_control_richedit($editor_id, $display_controls = 'all') {
  ...
  if ($display_controls !== 'all' && !is_array($display_controls)) $display_controls = array($display_controls);
...
function template_control_verification($verify_id, $display_type = 'all', $reset = false) {
  ...
  if (empty($verify_context['tracking']) || $reset) $verify_context['tracking'] = 0;
  ...
  if ($verify_context['tracking'] > $total_items) return false;
  
  // Loop through each item to show them.
  for ($i = 0; $i < $total_items; $i++) {
    
    // If we're after a single item only show it if we're in the right place.
    if ($display_type == 'single' && $verify_context['tracking'] != $i) continue;
    ...
    if ($display_type == 'single' && $verify_context['tracking'] == $i) break;
  }
