...
(function (window, document, clik, Zepto, jQuery) {
	...
	TouchController.prototype.multiHandDestroy = function (fireEvents) {
		if ( !this.multiHand ) {
			return;
		}
		...
		forEach(this.multiHand.fingers, function (finger) {
			...
			points.push(point);
			if (fireEvents) { finger.trigger('end', point); }
		});

		if (fireEvents) { this.multiHand.trigger('end', points); }
